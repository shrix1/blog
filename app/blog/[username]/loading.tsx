  ...
  return (
    <main className="w-full h-[90vh] py-6 flex flex-col gap-3 items-center">
      <section className="w-full lg:w-1/2 flex justify-center flex-col gap-3 items-center">
        <Skeleton className="w-24 h-24 rounded-full loading" />

        <div className="flex flex-col gap-2 justify-center items-center">
          <Skeleton className="w-36 p-3 loading" />
          <Skeleton className="w-48 p-2 loading" />
        </div>
      </section>
      ...
      <div className="flex justify-between flex-col gap-2 md:gap-0 md:flex-row w-full lg:w-1/2 items-center">
        <div className="flex gap-2">
          <Skeleton className="w-40 p-3 loading" />
          <Skeleton className="w-10 p-3 loading" />
        </div>
        <Skeleton className="w-36 p-3 loading" />
      </div>
      ...
      {Array(4)
        ...
        .map((_, index) => {
          return (
            ...
            >
              <Skeleton className="w-40 p-2 loading" />
              <Skeleton className="w-full p-4 loading" />

              <div className="flex flex-col gap-2 w-full">
                <Skeleton className="w-full p-2 loading" />
                <Skeleton className="w-[75%] p-2 loading" />
              </div>

              <div className="flex gap-4">
                <Skeleton className="w-20 p-2 loading" />
                <Skeleton className="w-20 p-2 loading" />
                <Skeleton className="w-20 p-2 loading" />
              </div>
            </section>
